...
export default function DevicesPage() {
  ...
  const [form, setForm] = useState({
    ...
    api_username: "admin", api_password: "", api_port: 443, api_ssl: true, description: "",
  });
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ name: "", ip_address: "", snmp_community: "public", snmp_port: 161, api_username: "admin", api_password: "", api_port: 443, api_ssl: true, description: "" });
    setDialogOpen(true);
  };
  ...
  const openEdit = (d) => {
    ...
    setForm({
      ...
      api_username: d.api_username || "admin", api_password: "", api_port: d.api_port || 443,
      api_ssl: d.api_ssl !== undefined ? d.api_ssl : true, description: d.description || "",
    });
    setDialogOpen(true);
  };
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="rounded-sm bg-card border-border max-w-lg" data-testid="device-dialog">
          ...
          <div className="space-y-5">
            ...
            {/* REST API Section */}
            <div className="border border-border/50 rounded-sm p-3 space-y-3">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1"><Zap className="w-3 h-3" /> REST API (RouterOS 7.1+)</p>
              <div className="grid grid-cols-2 gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label className="text-xs text-muted-foreground">API Port</Label>
                  <Input type="number" value={form.api_port} onChange={e => setForm({...form, api_port:e.target.value})} className="rounded-sm bg-background font-mono text-xs" data-testid="device-form-api-port" /></div>
                <div className="space-y-1.5"><Label className="text-xs text-muted-foreground">SSL</Label>
                  <Select value={form.api_ssl?"true":"false"} onValueChange={v => setForm({...form, api_ssl:v==="true"})}>
                    ...
                    <SelectContent><SelectItem value="true">HTTPS (443)</SelectItem><SelectItem value="false">HTTP (80)</SelectItem></SelectContent>
                  </Select></div>
              </div>
            </div>
